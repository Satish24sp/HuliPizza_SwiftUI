...
struct OrderDetailView: View {
    ...
    var body: some View {
        VStack{
            HStack {
                if let image = UIImage(named: "images-\(orderItem.item.id)"){
                    Image(uiImage: image)
                        .resizable()
                        .scaledToFit()
            ...
            .background(.linearGradient(colors: [.colors0, .colors1], startPoint: .leading, endPoint: .trailing), in:Capsule())
